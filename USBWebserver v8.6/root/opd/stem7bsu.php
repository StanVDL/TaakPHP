...
<title>Stem formulier 7BSU</title>
<link rel="stylesheet" type="text/css" href="tom.css">
<script type="text/javascript">

	function succes() {
	 location = "succes7bsu.php" ;
	}
	function faal() {
	 location = "faal7bsu.php" ;
	}
</script>
</head>
<div id="wrapper">
...
<h1>Stembiljet beste stand 7BSU</h1>
...
<?php
	...
	$query = mysql_query("SELECT * FROM stemmen7bsu") ;
		...
		echo "<tr>" ;
			...
			for ($j = 0; $j < $aantal_rows; $j++) {
				...
				if($nummer <= $aantal_standjes) {
					...
					$query = mysql_query("SELECT * FROM stemmen7bsu WHERE Bedrijfje=$nummer") ;
...
<?php
	...
	if (isset($_POST['Stem'])) {
		...
		if($teller != 3)
			...
			$query = mysql_query("SELECT * FROM stemmen7bsu WHERE bedrijfje = $stem1 OR bedrijfje = $stem2 OR bedrijfje = $stem3") ;
			
			while($stem = mysql_fetch_array($query)){
				$stem['Aantal']++ ;
				mysql_query("UPDATE stemmen7bsu SET Aantal = ".$stem['Aantal']." WHERE Bedrijfje = '".$stem['Bedrijfje']."'") ;
			}
