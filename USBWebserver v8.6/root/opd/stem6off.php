...
<title>Stem formulier 6OFF</title>
<link rel="stylesheet" type="text/css" href="tom.css">
<script type="text/javascript">

	function succes() {
	 location = "succes6off.php" ;
	}
	function faal() {
	 location = "faal6off.php" ;
	}
</script>
</head>
<div id="wrapper">
...
<h1>Stembiljet beste stand 6OFF</h1>
...
<?php
	...
	$query = mysql_query("SELECT * FROM stemmen6off") ;
		...
		echo "<tr>" ;
			...
			for ($j = 0; $j < $aantal_rows; $j++) {
				...
				if($nummer <= $aantal_standjes) {
					...
					$query = mysql_query("SELECT * FROM stemmen6off WHERE Bedrijfje=$nummer") ;
...
<?php
	...
	if (isset($_POST['Stem'])) {
		...
		if($teller != 3)
			...
			$query = mysql_query("SELECT * FROM stemmen6off WHERE bedrijfje = $stem1 OR bedrijfje = $stem2 OR bedrijfje = $stem3") ;
			
			while($stem = mysql_fetch_array($query)){
				$stem['Aantal']++ ;
				mysql_query("UPDATE stemmen6off SET Aantal = ".$stem['Aantal']." WHERE Bedrijfje = '".$stem['Bedrijfje']."'") ;
			}
